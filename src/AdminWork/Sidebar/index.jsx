...
const Sidebar = () => {
    ...
    const pages = [
        { name: 'AdminPage1', path: `/${RouteNames.ADMINPAGE1}`, Icon: DashboardIcon, ActiveIcon: DashboardIconWhite },
        { name: 'AdminPage2', path: `/${RouteNames.ADMINPAGE2}`, Icon: ProjectsIcon, ActiveIcon: ProjectsIconWhite },
                    ...
                    <List className={styles.list}>
                        {pages.map(({ name, Icon, ActiveIcon, path }) => (
                            <React.Fragment key={name}>
                                <ListItem disablePadding className={`${styles.listItem} ${(name === 'AdminPage1') || currentPath === path ? styles.activeItem : ''}`}>

                                    <ListItemButton onClick={() => handleClick(path)} className={styles.listItemBtn}>
                                        <ListItemIcon className={styles.listItemIcon}>
                                            <img
                                                src={(name === 'AdminPage1') || currentPath === path ? ActiveIcon : Icon}
                                                alt={name}
