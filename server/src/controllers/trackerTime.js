...
const checkIn = asyncHandler(async (req, res) => {
    ...
    let timeEntry = await userTracker.findOne({
        userId,
        projectId,
        date: today,
    });
...
const getElapsedTime = asyncHandler(async (req, res) => {
    ...
    if (timer.checkIn && timer.isRunning) {
        elapsedTime =
            Math.floor((Date.now() - new Date(timer.checkIn)) / 1000) -
...
const checkOut = asyncHandler(async (req, res) => {
    ...
    if (timeEntry.isRunning && timeEntry.lastPaused) {
        throw new apiError(
            400,
            "Cannot check out while paused. Resume before checking out."
        );
    }

    const checkOutTime = new Date();
    let totalWorkedTime = Math.floor(
...
const getUserTimeProject = asyncHandler(async (req, res) => {
    ...
    const timeEntries = await userTracker.find({
        userId,
        projectId,
        date: selectedDate,
    });
    ...
    const getUserTime = await userTracker
        .find({ userId, projectId, date: selectedDate })
        .select(
            "userId projectId checkIn isRunning isCheckedOut totalDuration maxTime effectiveElapsedTime date"
        )
    ...
    res.status(200).json(
        new apiResponse(
            200,
            { projectId, date: selectedDate, getUserTime },
...
const getUsersTimeProject = asyncHandler(async (req, res) => {
    ...
    const timeEntries = await userTracker
        .find({ projectId })
        .select(
            "projectId userId totalDuration pausedDuration lastPaused maxTime"
        );

    if (timeEntries.length === 0) {
        return res
            .status(404)
            .json(new apiResponse(404, [], "No users found for this project."));
    }
