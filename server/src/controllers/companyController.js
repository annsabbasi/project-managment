...
const refreshAccessToken = asyncHandler(async (req, res) => {
    const incomingRefreshToken = req.cookies?.refreshTokenC || req.body.refreshTokenC;
    ...
    try {
        ...
        return res.status(200)
            ...
            .cookie("role", company?.role, options)
...
const registerCompany = asyncHandler(async (req, res) => {
    ...
    const createdCompany = await Company.findById(registerCompany._id).select(
...
const loginCompany = asyncHandler(async (req, res) => {
    ...
    return res.status(200)
        .cookie('role', loggedInCompany.role, options)
        .cookie('accessTokenC', accessToken, options)
        .cookie('refreshTokenC', refreshToken, options)
        .json(
            new apiResponse(200, {
...
const getAllData = asyncHandler(async (req, res) => {
    ...
    if (data.length === 0) {
        throw new apiError(404, "No companys found");
    }
    return res.status(200).json(
...
const getCompanyProfile = asyncHandler(async (req, res) => {
    const token = req.cookies?.accessTokenC;
...
// Promoting a Company To QC-Admin
const promoteUser = asyncHandler(async (req, res) => {
    ...
    if (req.company.role !== ROLES.ADMIN) {
        throw new apiError(403, "You don't have to authority to perform this action");
    }

    const companyToPromote = await Company.findById(companyId);
    if (!companyToPromote) {
        throw new apiError(404, "Company not found.");
    }
    if (companyToPromote.role === ROLES.QCADMIN) {
        return res.status(400).json(new apiResponse(400, {}, "Company is already a QcAdmin."));
    }

    companyToPromote.role = ROLES.QCADMIN
    await companyToPromote.save();

    return res.status(200).json(new apiResponse(200, { companyId: companyToPromote._id, role: companyToPromote.role }, "Company promoted to QcAdmin successfully."))
})
